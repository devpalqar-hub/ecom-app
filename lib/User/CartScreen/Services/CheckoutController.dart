...
import 'package:new_project/User/CartScreen/Model/AddressModel.dart';
...
class CheckoutController extends GetxController {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (selectedAddress.value == null && defaultAddress != null) {
          ...
          checkDelivery(selectedAddress.value!.postalCode ?? "");
  ...
  Future<Map<String, dynamic>?> checkDelivery(String postalCode) async {
    CartController cctrl = Get.find();
    try {
      final response = await http.get(
        Uri.parse(
          "$baseUrl/orders/check/delivery?postalCode=$postalCode&orderAmount=${cctrl.getPayableTotal()}",
        ),
        headers: {
